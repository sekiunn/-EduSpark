...
import wordIcon from '@/assets/images/word_en.svg'
...
const resolvedTitle = computed(() => props.cardData?.title || '教案工作区')
...
const previewLines = computed(() => buildPreviewLines(
  props.cardData?.preview || props.cardData?.summary,
  ['教案内容概览', '进入工作区继续编辑', '支持保存与导出']
))
</script>

<template>
  <div
    class="lesson-stage-card"
  ...
  >
    <div class="entry-content">
      <img :src="wordIcon" alt="" class="entry-icon" />
    ...
    <div class="entry-preview" aria-hidden="true">
      <div class="entry-preview-sheet">
        <div class="entry-preview-title">教案文档</div>
        <div
...
<style scoped>
.lesson-stage-card {
  --entry-accent-rgb: 236, 102, 48;
  --entry-progress-rgb: 236, 102, 48;
...
.lesson-stage-card:hover:not(:disabled) {
...
.lesson-stage-card.is-disabled {
  cursor: not-allowed;
  opacity: 0.7;
}

.lesson-stage-card:focus-visible {
...
.lesson-stage-card.is-preparing .entry-meta-dot,
.lesson-stage-card.is-retrieving .entry-meta-dot,
.lesson-stage-card.is-enriching .entry-meta-dot,
.lesson-stage-card.is-drafting .entry-meta-dot {
  background: rgb(var(--entry-progress-rgb));
  box-shadow: 0 0 0 2px rgba(var(--entry-progress-rgb), 0.10);
}

.lesson-stage-card.is-preparing .entry-meta-dot::before,
.lesson-stage-card.is-retrieving .entry-meta-dot::before,
.lesson-stage-card.is-enriching .entry-meta-dot::before,
.lesson-stage-card.is-drafting .entry-meta-dot::before {
  border-color: rgba(var(--entry-progress-rgb), 0.18);
  animation: entryRipplePulse 2.2s cubic-bezier(0.22, 1, 0.36, 1) infinite;
}

.lesson-stage-card.is-preparing .entry-meta-dot::after,
.lesson-stage-card.is-retrieving .entry-meta-dot::after,
.lesson-stage-card.is-enriching .entry-meta-dot::after,
.lesson-stage-card.is-drafting .entry-meta-dot::after {
  border-color: rgba(var(--entry-progress-rgb), 0.1);
  animation: entryRipplePulse 2.2s cubic-bezier(0.22, 1, 0.36, 1) infinite 1.1s;
}

.lesson-stage-card.is-completed .entry-meta-dot {
  background: #22c55e;
}

.lesson-stage-card.is-failed .entry-meta-dot {
  background: #ef4444;
}
